...
class TestCreateUser:
    ...
    def test_can_create_user(self, create_user_payload, del_user):
        ...
        del_user.update({'token': user['accessToken']})
        assert user["success"]
    ...
    def test_cant_create_user_dupes(self, create_user_payload, make_user):
        ...
        resp_dupe = UserRequests().post_create_user(data=payload, status=403)
        assert resp_dupe["message"] == "User already exists"
    ...
    def test_all_the_fields_are_required(self, payload_schema, create_user_payload):
        ...
        resp = UserRequests().post_create_user(data=payload, status=403)
        assert resp["message"] == "Email, password and name are required fields"
